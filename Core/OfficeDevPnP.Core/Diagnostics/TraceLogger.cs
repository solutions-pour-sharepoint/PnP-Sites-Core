...
namespace OfficeDevPnP.Core.Diagnostics
{
    class TraceLogger : ILogger
    {
        ...
        private string GetLogEntry(LogEntry entry, LogLevel level)
        {
            try
            {
                var log = string.Format("{0}\t[{1}]\t[{2}]\t[{3}]\t{4}\t{5}ms\t{6}", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff"), entry.Source, entry.ThreadId, level.ToString(), entry.Message, entry.EllapsedMilliseconds, entry.CorrelationId != Guid.Empty ? entry.CorrelationId.ToString() : "");
